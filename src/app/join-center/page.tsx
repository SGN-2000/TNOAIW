...
type Step = "enterCode" | "confirm" | "verify" | "selectCourse" | "final";
...
export default function JoinCenterPage() {
    ...
    useEffect(() => {
        if (!authLoading && profileComplete === false) {
            router.push('/');
        }
    ...
    if (!user) {
        // Should be handled by MainLayout, but as a fallback
        return <Loader />;
    }
    ...
    const handleSearchCode = async () => {
        ...
        try {
            ...
            snapshot.forEach((centerSnapshot) => {
                ...
                if (centerData.codes) {
                    ...
                    if (role && centerId) {
                        ...
                        const isMember = (centerData.members?.admins && centerData.members.admins[user.uid]) || (centerData.members?.students && centerData.members.students[user.uid]) || (centerData.members?.adminsPlus && centerData.members.adminsPlus[user.uid]) || centerData.ownerId === user.uid;
                        if(isMember) {
                             toast({ title: "Ya eres miembro", description: `Ya perteneces al centro "${centerData.centerName}".`, variant: "destructive" });
                             centerFound = null; // Prevent joining
                             return; // exit forEach
                        }
            ...
            if (centerFound) {
                ...
                toast({ title: "Código Inválido", description: "No se encontró ningún centro con ese código o ya eres miembro.", variant: "destructive" });
            }
        } catch (error) {
