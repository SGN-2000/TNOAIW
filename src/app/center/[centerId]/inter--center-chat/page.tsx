...
import { Globe, Users, MessageSquare, MessageSquarePlus } from 'lucide-react';
...
export default function InterCenterChatPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const potentialMemberIds = [...new Set([...adminIds, ...adminPlusIds])];
      ...
      const userChatsRef = ref(db, `userInterCenterChats/${centerId}`);
  ...
  const handleStartDirectChat = async (targetCenter: CenterProfile) => {
    if(!centerId || !user) return;
    try {
        const newChat = await createDirectChat(centerId, user.displayName || "Unknown User", targetCenter.id, targetCenter.centerName);
            ...
            <Dialog open={isCreateGroupOpen} onOpenChange={setCreateGroupOpen}>
                ...
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>Crea un chat grupal con múltiples centros.</DialogDescription>
                    </DialogHeader>
                    <CreateGroup
