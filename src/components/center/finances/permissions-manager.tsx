...
export default function PermissionsManager({ centerId, currentManagers, potentialManagers, onSave }: PermissionsManagerProps) {
  ...
  const handleSave = async () => {
    ...
    const newManagerIds = new Set(selectedManagers);

    const newlyAssigned = selectedManagers.filter(id => !previousManagerIds.has(id));
    const newlyRemoved = [...previousManagerIds].filter(id => !newManagerIds.has(id));
    ...
    try {
      ...
      await update(ref(db, `centers/${centerId}/finances/permissions`), { managers: managersObject });
      ...
      const notificationPromises = [
        ...newlyAssigned.map(id => set(push(ref(db, `notifications/${id}`)), {
            type: 'NEW_PERMISSION', centerId, centerName, permissionType: 'FINANCE_MANAGER', timestamp: new Date().toISOString(), read: false
        })),
        ...newlyRemoved.map(id => set(push(ref(db, `notifications/${id}`)), {
            type: 'REMOVED_PERMISSION', centerId, centerName, permissionType: 'FINANCE_MANAGER', timestamp: new Date().toISOString(), read: false
        })),
      ];
      await Promise.all(notificationPromises);

      toast({ title: "Gestores actualizados", description: "Se han guardado los nuevos gestores de finanzas." });
      onSave();
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="space-y-6 pt-4">
        <p className="text-sm text-muted-foreground">Elige qué administradores pueden añadir, editar y eliminar transacciones financieras.</p>
