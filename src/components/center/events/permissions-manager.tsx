...
export default function PermissionsManager({ centerId, potentialManagers, onSave }: PermissionsManagerProps) {
  ...
  useEffect(() => {
    const permsRef = ref(db, `centers/${centerId}/events/permissions/managers`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await update(ref(db, `centers/${centerId}/events/permissions`), { managers: managersObject });
      
      toast({ title: "Permisos actualizados", description: "Se han guardado los nuevos gestores de eventos." });
      onSave();
    } catch (error) {
      console.error(error);
