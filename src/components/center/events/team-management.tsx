...
const ManagerTeamBuilder = ({ event, centerId, allCenterMembers }: { event: Event; centerId: string; allCenterMembers: CenterMember[] }) => {
    ...
    useEffect(() => {
        const assignedMemberIds = new Set(teams.flatMap(t => Object.keys(t.members)));
                     ...
                     {teams.map(team => (
                        <Card key={team.id}>
                            ...
                            <CardContent className="space-y-2">
                                ...
                                {Object.keys(team.members).length === 0 && <p className="text-xs text-muted-foreground text-center p-2">Equipo vacío</p>}
                            </CardContent>
                             <CardFooter>
                                <Popover>
                                    <PopoverTrigger asChild>
