...
import { MoreHorizontal, User, Shield, ShieldCheck, UserMinus, UserCog, Trophy, UserPlus, Banknote, Eye, ShieldAlert, Store, XCircle, MessageCircle, Globe, Check, Users } from "lucide-react";
...
export interface Notification {
  id: string;
  type: 'NEW_MEMBER' | 'NEW_PERMISSION' | 'ROLE_CHANGED' | 'FINANCE_VISIBILITY_CHANGED' | 'REMOVED_PERMISSION' | 'STORE_STATUS_CHANGED' | 'RESERVATION_CANCELED' | 'NEW_ANONYMOUS_CHAT' | 'NEWS_TEAM_MEMBER' | 'GROUP_INVITE';
  centerId: string;
  centerName?: string;
...
export default function NotificationCard({ notification, onMarkAsRead, currentUserRole }: NotificationCardProps) {
    ...
    const handleKickMember = async () => {
     ...
     const handleGroupInvite = async (accept: boolean) => {
        ...
        try {
            // Mark the notification as read to hide it
            await update(ref(db, `notifications/${user.uid}/${id}`), { read: true }); 
            
            if (accept) {
                const currentCenterSnap = await get(ref(db, `centers/${invitedCenterId}`));
                if (!currentCenterSnap.exists()) {
                    throw new Error("El centro actual no existe");
                }
                
                // Add current center to group members and user-chat link
                const updates: { [key: string]: any } = {};
    ...
    const renderContent = () => {
        switch (notification.type) {
                            ...
                            <DropdownMenu onOpenChange={(open) => { if(open) handleInteraction() }}>
                                ...
                                <DropdownMenuContent align="end">
                                    ...
                                    <DropdownMenuItem asChild>
                                        <Link href={`/center/${notification.centerId}/profile/${notification.subjectUserId}`}>
                                    ...
                                    <DropdownMenuSub>
                                        ...
                                        <DropdownMenuSubContent>
                                            {currentUserRole === 'owner' && <DropdownMenuItem onClick={() => handleRoleChange('admin-plus')}>
                                    ...
                                    <DropdownMenuSeparator />
                                    <DropdownMenuItem className="text-destructive" onClick={handleKickMember}>
                     ...
                     <Card className={cn("shadow-sm relative", !notification.read && "bg-primary/5")} onClick={handleInteraction}>
                        {!notification.read && <div className="absolute top-3 right-3 h-2.5 w-2.5 rounded-full bg-primary" />}
                         <CardContent className="p-3 flex-row flex items-center gap-4 space-y-0">
                             ...
                             <div className="flex-1">
                                <p className="text-sm">
                                     Has sido designado como <span className="font-semibold">{notification.permissionType && permissionLabels[notification.permissionType]}</span> en {notification.centerName}.
                                </p>
                                 <p className="text-xs text-muted-foreground">
                                    {formatDistanceToNow(new Date(notification.timestamp), { addSuffix: true, locale: es })}
                                </p>
            ...
            case 'REMOVED_PERMISSION':
                 return (
                     <Card className={cn("shadow-sm relative", !notification.read && "bg-destructive/10")} onClick={handleInteraction}>
                        {!notification.read && <div className="absolute top-3 right-3 h-2.5 w-2.5 rounded-full bg-destructive" />}
                         <CardContent className="p-3 flex-row flex items-center gap-4 space-y-0">
                             <div className="p-2 bg-muted rounded-full">{getIcon()}</div>
                             <div className="flex-1">
                                <p className="text-sm">
                                     Te han quitado los permisos de <span className="font-semibold">{notification.permissionType && permissionLabels[notification.permissionType]}</span> en {notification.centerName}.
                                </p>
                                 <p className="text-xs text-muted-foreground">
                                    {formatDistanceToNow(new Date(notification.timestamp), { addSuffix: true, locale: es })}
                                </p>
                     ...
                     <Card className={cn("shadow-sm relative", !notification.read && "bg-primary/5")} onClick={handleInteraction}>
                        {!notification.read && <div className="absolute top-3 right-3 h-2.5 w-2.5 rounded-full bg-primary" />}
                         <CardContent className="p-3 flex-row flex items-center gap-4 space-y-0">
                             ...
                             <div className="flex-1">
                                <p className="text-sm">
                                     Tu rol en {notification.centerName} ha sido cambiado a <span className="font-semibold">{notification.newRole && roleLabels[notification.newRole]}</span>.
                                </p>
                                 <p className="text-xs text-muted-foreground">
                                    {formatDistanceToNow(new Date(notification.timestamp), { addSuffix: true, locale: es })}
                                </p>
                     ...
                     <Card className={cn("shadow-sm relative", !notification.read && "bg-primary/5")} onClick={handleInteraction}>
                        {!notification.read && <div className="absolute top-3 right-3 h-2.5 w-2.5 rounded-full bg-primary" />}
                         <CardContent className="p-3 flex-row flex items-center gap-4 space-y-0">
                             ...
                             <div className="flex-1">
                                <p className="text-sm">
                                     La visibilidad de las finanzas en {notification.centerName} ha cambiado a: <span className="font-semibold">{notification.newVisibility}</span>.
                                </p>
                                 <p className="text-xs text-muted-foreground">
                                    {formatDistanceToNow(new Date(notification.timestamp), { addSuffix: true, locale: es })}
                                </p>
                     ...
                     <Card className={cn("shadow-sm relative", !notification.read && "bg-primary/5")} onClick={handleInteraction}>
                        {!notification.read && <div className="absolute top-3 right-3 h-2.5 w-2.5 rounded-full bg-primary" />}
                         <CardContent className="p-3 flex-row flex items-center gap-4 space-y-0">
                             ...
                             <div className="flex-1">
                                <p className="text-sm">
                                     La tienda de {notification.centerName} ha sido <span className="font-semibold">{notification.newStatus}</span>.
                                </p>
                                 <p className="text-xs text-muted-foreground">
                                    {formatDistanceToNow(new Date(notification.timestamp), { addSuffix: true, locale: es })}
                                </p>
                     ...
                     <Card className={cn("shadow-sm relative", !notification.read && "bg-destructive/10")} onClick={handleInteraction}>
                        {!notification.read && <div className="absolute top-3 right-3 h-2.5 w-2.5 rounded-full bg-destructive" />}
                         <CardContent className="p-3 flex-row flex items-center gap-4 space-y-0">
                             ...
                             <div className="flex-1">
                                <p className="text-sm">
                                     Tu reserva del producto <span className="font-semibold">{notification.productName}</span> ha sido cancelada por un administrador.
                                </p>
                                 <p className="text-xs text-muted-foreground">
                                    {formatDistanceToNow(new Date(notification.timestamp), { addSuffix: true, locale: es })}
                                </p>
                     ...
                     <Card className={cn("shadow-sm relative", !notification.read && "bg-primary/5")} onClick={handleInteraction}>
                        {!notification.read && <div className="absolute top-3 right-3 h-2.5 w-2.5 rounded-full bg-primary" />}
                         <CardContent className="p-3 flex-row flex items-center gap-4 space-y-0">
                             ...
                             <div className="flex-1">
                                <p className="text-sm">
                                     Un nuevo <span className="font-semibold">chat anónimo</span> ha sido iniciado en {notification.centerName}.
                                </p>
                                 <p className="text-xs text-muted-foreground">
                                    {formatDistanceToNow(new Date(notification.timestamp), { addSuffix: true, locale: es })}
                                </p>
