...
**FOR TESTING**: Instead of using the provided team names, generate generic team names like "Equipo 1", "Equipo 2", etc. The number of teams should match the input.
...
2.  **Distribute Teams**: If there are groups, distribute the generated "Equipo X" teams among them as evenly as possible.
...
7.  Do not invent teams beyond the required number.
...
const generateFixtureFlow = ai.defineFlow(
  ...
  async (input) => {
    // Generate test teams
    const testTeams = Array.from({ length: input.teams.length }, (_, i) => ({
      id: `team_${i + 1}`,
      name: `Equipo ${i + 1}`,
    }));
